...
export class FakeBackendInterceptor implements HttpInterceptor {
    ...
    intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
        ...
        function authenticate() {
            ...
            const account = accounts.find(x => x.email.toLowerCase() === email.toLowerCase() && x.password === password);

            if (!account) return error('Email or password is incorrect');

            if (!account.isVerified) return error('Account is not verified');
        ...
        function register() {
            ...
            if (accounts.find(x => x.email.toLowerCase() === account.email.toLowerCase())) {
                setTimeout(() => {
                    alertService.info(`
            ...
            account.isVerified = true; // Auto-verify for ease of testing in prototype
            ...
            setTimeout(() => {
                alertService.success('Registration successful, you can now login', { keepAfterRouteChange: true });
            }, 1000);
        ...
        function updateAccount() {
            ...
            if (params.email && accounts.find(x => x.email.toLowerCase() === params.email.toLowerCase() && x.id !== account.id)) {
